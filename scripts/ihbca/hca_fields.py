...
from ihbca.constants import DATASET_META_FIELDS, DATASET_TO_STUDY
...
def get_study_from_donor_id(donor_id):
...
def populate_dataset_metadata(adata, study, repo_root):
    """Load per-study dataset metadata from YAML and set uns fields."""
    dataset_meta = load_dataset_metadata(repo_root)
    if not dataset_meta:
...
def collect_study_pis(repo_root):
    ...
    """
    meta = load_dataset_metadata(repo_root)
    if not meta:
...
def collect_study_accessions(repo_root):
