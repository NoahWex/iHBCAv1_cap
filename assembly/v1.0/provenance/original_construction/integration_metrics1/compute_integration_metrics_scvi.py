...
# Here we look at scVI integration
...
####### Compute the scVI integration metrics #####

scvi_dir = '/home/adr44/rds/hpc-work/hbca/data/integration/scvi/output/iHBCA/global/'
...
scvi_cell_data = pd.read_csv(scvi_dir + 'n_dims_' + str(n_dims) + '/cell_data.csv', index_col=0, dtype=str)
cells_to_keep = scvi_cell_data.index.isin(adata_integration.obs.index)
adata_integration.obs = scvi_cell_data[cells_to_keep]
#add scvi embedding
scvi_emb = pd.read_csv(scvi_dir + 'n_dims_' + str(n_dims) + '/X_scVI.csv', index_col=0)
adata_integration.obsm['X_emb'] = scvi_emb.iloc[cells_to_keep,].values
...
#Save the metrics
os.makedirs(save_dir + 'scvi_metrics/', exist_ok=True)
integration_metrics.to_csv(save_dir + 'scvi_metrics/' + 'n_dims_' + str(n_dims) + '_integration_metrics.csv')
