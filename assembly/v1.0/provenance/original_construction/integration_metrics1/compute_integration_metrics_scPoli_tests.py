...
save_dir = '/home/adr44/rds/hpc-work/hbca/data/integration/integration_comparison/' + output_dir +'/' + subset + '/'
...
scpoli_dir = '/home/adr44/rds/hpc-work/hbca/data/integration/scPoli/' + output_dir + '/'

latent_dim = int(sys.argv[3])
hidden_layer_size = int(sys.argv[4])
print(latent_dim)
...
#Add UMAPs to the scPoli object
scpoli_umap = pd.read_csv(scpoli_dir + '/n_dims_' + str(latent_dim) + '/UMAP.csv', index_col=0)
adata_integration.obsm['X_scpoli_umap'] = scpoli_umap.iloc[cells_to_keep_bool,].values

# plot some UMAPs to check data merging
sc.settings.figdir = save_dir + 'test_umaps/'
sc.pl.umap(adata, color=['harmonise_level1', 'leiden_0_5', 'dataset'], ncols=1, save='_original.png')
sc.pl.umap(adata_integration, color=['harmonise_level1', 'leiden_0_5', 'dataset'], ncols=1, save='_scPoli.png')
sc.pl.embedding(adata_integration, basis='X_scpoli_umap', color=['harmonise_level1', 'leiden_0_5', 'dataset'], ncols=1, save='_scPoli_umap_emb.png')
