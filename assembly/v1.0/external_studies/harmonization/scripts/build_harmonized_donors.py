...
def derive_parity_binary(row: pd.Series) -> Optional[str]:
    """
    Derive binary parity from parity count.

    Returns: 'nulliparous' (0), 'parous' (>0), or None
    """
    count = row.get('parity_count')

    if pd.notna(count):
        return 'nulliparous' if count == 0 else 'parous'
...
def extract_dissociation_minutes(row: pd.Series) -> Optional[int]:
    """
    Extract dissociation time in minutes (Reed only).
...
def derive_menopausal_binary(row: pd.Series) -> Optional[str]:
    """
    Derive binary menopausal status from detailed status.

    Mapping:
    - pre, peri -> 'pre'
    - post_natural, post_surgical -> 'post'
    """
...
def build_metadata_notes(row: pd.Series) -> Optional[str]:
    ...
    if row.get('tissue_indication') == 'unknown':
        if study == 'twigger':
...
def harmonize_parity_binary(row: pd.Series) -> Optional[str]:
    """Derive binary parity (nulliparous/parous) from study-specific columns."""
    study = row['ihbca_dataset']
    ...
    if study == 'gray':
        ...
        if pd.notna(val) and str(val).strip() not in ['', 'nan', 'NA']:
            try:
                births = float(val)
                return 'nulliparous' if births == 0 else 'parous'
            except (ValueError, TypeError):
                return None
        ...
        if parity is not None:
            return 'nulliparous' if parity == 0 else 'parous'
        return None
        ...
        # Reed uses ihbca_parous (mixed encoding)
...
def harmonize_menopausal_status(row: pd.Series) -> Optional[str]:
    """Derive binary menopausal status (pre/post) from study-specific columns."""
    study = row['ihbca_dataset']
    ...
    if study == 'gray':
        ...
        # All donors age 19-47 (premenopausal) - no testable contrast
        return 'pre'
...
def main():
    ...
    print("\ndissociation_minutes (Reed only):")
    ...
    for col in ['age_continuous', 'age_binary', 'parity_count', 'parity_binary', 'age_at_first_birth',
                ...
                'sample_preservation', 'sample_type', 'facs_status', 'dissociation_minutes']:
